...
function Series() {
  ...
  useEffect(() => {
    ...
    GlobalApi.getTopRatedSeries().then((resp) => {

      setSeriesData((prev) => ({
        ...prev,
        "Top Rated Series": resp.data.results,
      }));
    });

    SERIES_SECTIONS.forEach((section) => {

      if (section.genre) {

        GlobalApi.getSeriesByGenre(section.genre)
          .then((resp) => {
    ...
    <div className="bg-[#040714] text-white min-h-screen overflow-hidden">
      ...
      {heroSeries && (
        ...
        <div className="relative h-[100svh] md:h-screen overflow-hidden">
          ...
          <img
            src={`${IMAGE_BASE_URL}${heroSeries.backdrop_path}`}
            className="
            absolute inset-0
            w-full h-full
            object-cover
        ...
        {SERIES_SECTIONS.map((section) => (
          ...
          >
            ...
            >
              ...
              {seriesData[section.title]?.map((movie) => (
